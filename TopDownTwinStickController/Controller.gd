...
const ATTACK = preload("res://TopDownTwinStickController/ATTACK.tscn")
...
func _ready():
	...
	attackPosition = MeshInstance.get_child(0)
	...
	get_node('HBoxContainer3/VBoxContainer/HBoxContainer2/Problem Label').set_text(mathProblem)
...
func _process(delta):
	...
	if(Input.is_action_just_pressed('attack')):
		attack()
	if (Input.is_action_just_pressed("Enter Number 0")):
		playerAnswer = playerAnswer + '0'
	if (Input.is_action_just_pressed("Enter Number 1")):
		playerAnswer = playerAnswer + '1'
	if (Input.is_action_just_pressed("Enter Number 2")):
		playerAnswer = playerAnswer + '2'
	if (Input.is_action_just_pressed("Enter Number 3")):
		playerAnswer = playerAnswer + '3'
	if (Input.is_action_just_pressed("Enter Number 4")):
		playerAnswer = playerAnswer + '4'
	if (Input.is_action_just_pressed("Enter Number 5")):
		playerAnswer = playerAnswer + '5'
	if (Input.is_action_just_pressed("Enter Number 6")):
		playerAnswer = playerAnswer + '6'
	if (Input.is_action_just_pressed("Enter Number 7")):
		playerAnswer = playerAnswer + '7'
	if (Input.is_action_just_pressed("Enter Number 8")):
		playerAnswer = playerAnswer + '8'
	if (Input.is_action_just_pressed("Enter Number 9")):
		playerAnswer = playerAnswer + '9'
	# Answer Submitted
	if answer == int(playerAnswer):
		playerAnswer = ''
		attack()
		createNewMathProblem()
		get_node('HBoxContainer3/VBoxContainer/HBoxContainer3/Answer Label').set_text(playerAnswer)
	elif String(answer).length() == playerAnswer.length():
		playerAnswer = ''
		createNewMathProblem()
		get_node('HBoxContainer3/VBoxContainer/HBoxContainer3/Answer Label').set_text(playerAnswer)
	else: get_node('HBoxContainer3/VBoxContainer/HBoxContainer3/Answer Label').set_text(playerAnswer)
	#Jump
...
func attack():
	...
	get_node("/root/").add_child(attack)
	attack.set_translation(attackPosition.get_global_transform().origin)
	attack.direction = attack.get_global_transform().basis.z
	
func createNewMathProblem():
	rng.randomize()
	num1 = rng.randi_range(0, 10.0)
	num2 = rng.randi_range(0, 10.0)
	answer = num1 + num2
